...
- 修复 T+ L 键长按会把候选标签 `0 l L` 整串提交的问题。根因是多候选 action 未放在带空格分隔符的 `softkey_list` 中；现在 L/M 单键及全部双字母键均按 Google 原生资源结构解析为独立候选。

### Testing

- Android 14 / API 34 / 4 KB 模拟器确认：L 长按弹出 `L / l / 0` 三个独立候选，原位松手仅提交 `0`；OP 长按仅提交 `5`，无 ART/Verify 错误。
